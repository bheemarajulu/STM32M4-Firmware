 ...
 * received data bit by bit */
 ...
 {
 uint8_t cur_bit,scr_bit,bspp;
	static uint8_t bs=0,bsp;
	 bspp=bsp;
	 bsp=bs;
 scr_bit = bit;
...
if((buf->byte != 0x7E)|((bspp==1)&&(buf->byte == 0x7E)))
 { //printf("\r \n data..\n"); 
 ...
 * Can filter out som possible bogus data */
 buf->data_detected = FALSE;
 if((buf->data_bytes > MIN_PACKET) && (buf->data_bytes < MAX_PACKET))
 {
...
if((bs==0) && (buf->byte == 0x7E) && (buf->flag_detected == FALSE))
{
...
/* Make space for new bit */
 ...
 {
 uint16_t i, j, more_addresses, valid;

 more_addresses = TRUE;
 valid = FALSE;
 i = 0;
 j = 0;
  printf("\r\n Received address bytes are \n");
